...
Last commit: `2cf4bd8` → `44214e6` → ... (see `git log`).
...
Astro + MDX + React islands. 8 static pages at build time. Dogfoods `@cathode-ui/react/tokens.css` — the docs chrome respects the same theme variables as the components it documents.
...
- `/components/chat` — live streaming `Chat` demo with mock provider
- `/components/dialog` — live `Dialog` demo (info + danger variants)
...
**Build**: `npm run build` — ~1.2 s, outputs pure static HTML + two small React island bundles.
